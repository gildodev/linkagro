...
# class costumizado de paginacao
class CustomPaginator(PageNumberPagination):
    page_size = 1  # Número de itens por página (pode ser ajustado)
    # page_size_query_param = "page_size"  # Permite ao usuário definir o tamanho da página
    # max_page_size = 50  # Define o limite máximo por página

# todos os produtos da categoria
@api.get("produtos-categoria/{slug_categoria}/")
# @paginate(CustomPaginator)
def get_produto_categoria(request, slug_categoria: str):
    ...
    for variedade in variedades_ativas:
...
def get_produto_varirdades(request, slug_produto: str):
    # Obter o produto com base no slug_produto
    ...
    # Gerando o link dinâmico para os detalhes da variedade
    # detalhes_link = reverse('variedade-detail', kwargs={'slug_variedade': slug_variedade}, request=request)
    variedades=[]
    

    for variedade in produto.produto_variedades.all():
